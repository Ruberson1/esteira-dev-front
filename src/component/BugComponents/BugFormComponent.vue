...
import CategoryList from "../../entity/CategoryList";
...
const categoryList: any = reactive(new CategoryList());
...
const taskId: any = route.params.taskId;
...
const isProductionToggle = () => {
  isProduction.value = !isProduction.value;
}


const submitForm = async () => {
  const production: any = isProduction.value === true ? 1 : 0;
  ...
  formData.append('tests', tests.value);
  ...
  if (dataGatewayPromise) {
    try {
      ...
      const bugData =  dataGateway.create('bug', formData);
      bugList.createBug(bugData);
    } catch (error) {
...
const updateForm = async () => {
  const formData = new FormData();
  formData.append('user_id', userSelected.value);
  formData.append('category_id', categorySelected.value);
  formData.append('title', title.value);
  formData.append('tests', tests.value);
  formData.append('description', description.value);
  // selectedFiles.value.forEach((file: any, index: any) => {
  //   formData.append('files[]', file.file);
  // });
  ...
  if (dataGatewayPromise) {
    try {
      ...
      const bugData = await dataGateway.update(`bug/${bugId}`, formData);
      bugList.createBug(bugData);
      setTimeout(() => {
...
const getBug = ( async (bugId: string) => {
  try {
    const dataGateway = await dataGatewayPromise;
    const bugData = await dataGateway.getById(`bug/${bugId}`);
    bugList.createBug(bugData);
  } catch (error) {
    console.error("Error fetching dataGateway:", error);
  }
  userSelected.value = bugList.bugs[0].task.user_id;

  categorySelected.value = bugList.bugs[0].category.id;
  tests.value = bugList.bugs[0].tests;
  title.value = bugList.bugs[0].title;
  description.value = bugList.bugs[0].description;
  isProduction.value = bugList.bugs[0].production;
})
...
onMounted(async () => {
  ...
  if (dataGatewayPromise) {
    try {
      const dataGateway = await dataGatewayPromise;
      const categoryData = await dataGateway.getAll('categories');
      const usersData = await dataGateway.getAll('users');
      categoryList.addCategory(categoryData);
    ...
    <form @submit.prevent="">
      <div>
        <label class="relative inline-flex items-center mb-5 cursor-pointer">
          <input @click="isProductionToggle()"  type="checkbox" value="" class="sr-only peer">
          <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-cyan-300 dark:peer-focus:ring-cyan-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 dark:peer-checked:bg-cyan-400 peer-checked:bg-amber-400" ></div>
          <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">Is it in production?</span>
        </label>
      </div>
      <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div>
          <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a user</label>
          <Dropdown v-model="userSelected" :options="userList.users"
        ...
        <div>
          ...
          <Dropdown v-model="categorySelected" :options="categoryList.categories"
                    optionLabel="name"  class="p-inputtext-sm md:w-14rem" />
        </div>
      </div>
      <div class="mb-6">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bug title</label>
        <input rules="required" v-model="title" type="text" id="required" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
      ...
      <div class="mb-6">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tests</label>
        <input v-model="tests" type="text" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
      focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-600
      dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
               placeholder="write the tests performed here..." required>
      </div>
      <div class="mb-6">
        <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bug description</label>
      ...
      <div class="mb-6" v-if="master && !bugId">
        <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bug image</label>
      ...
      <div class="mb-16">
        <a v-if="bugId" @click="updateForm()"  class="cursor-pointer w-full text-white flex items-center justify-center bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600
        ...
        <a v-else @click="submitForm()" type="submit" class="cursor-pointer w-full text-white flex items-center justify-center bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600
